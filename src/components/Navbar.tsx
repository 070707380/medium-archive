...
import { Plus, Unlock, Database, Search, Shield, BarChart3, Layers, Compass, Shuffle } from 'lucide-react';
import { MediaItem } from '../types';

interface NavbarProps {
  items: MediaItem[];
  isAdmin: boolean;
  activeView: 'archive' | 'hornets' | 'rating_scale' | 'similar';
  onViewChange: (view: 'archive' | 'hornets' | 'rating_scale' | 'similar') => void;
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#0a0c10]/95 backdrop-blur-md border-b border-slate-800/80 transition-all">
      <div className="max-w-7xl mx-auto px-3 sm:px-5 lg:px-6">
        ...
        <div className="flex flex-col gap-2 py-2 md:hidden font-mono">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-8 h-8 rounded-lg bg-slate-900 border border-amber-500/30 flex items-center justify-center shadow-md hover:border-amber-400 transition shrink-0 group p-1.5"
                title="Medium Archive"
              >
                <svg viewBox="0 0 24 24" fill="none" className="w-full h-full text-amber-400" stroke="currentColor" strokeWidth="2">
                  <path d="M4 6h16M4 12h16M4 18h16" strokeLinecap="round" />
                  <rect x="7" y="3" width="10" height="18" rx="2" className="fill-amber-500/20 stroke-amber-400" />
                  <circle cx="12" cy="12" r="2" className="fill-amber-400" />
                </svg>
              </button>

              <h1
                onClick={() => onViewChange('archive')}
                className="text-xs font-black tracking-wider text-slate-100 uppercase cursor-pointer"
              >
                MEDIUM ARCHIVE
              </h1>
            </div>
          ...
          {/* Row 2: Equal 4-Column Navigation Segmented Bar */}
          <nav className="grid grid-cols-4 gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800/90 text-[11px] font-bold text-center">
            <button
              onClick={() => onViewChange('archive')}
            ...
            >
              <Compass size={13} className={activeView === 'similar' ? 'text-slate-950' : 'text-amber-400'} />
        ...
        <div className="hidden md:flex items-center justify-between py-2.5 gap-3 flex-wrap lg:flex-nowrap min-w-0">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2.5">
              <button
                onClick={() => onViewChange('archive')}
                className="w-9 h-9 rounded-lg bg-slate-900 border border-amber-500/30 flex items-center justify-center p-2 shadow-md hover:border-amber-400 transition shrink-0"
                title="Medium Archive"
              >
                <svg viewBox="0 0 24 24" fill="none" className="w-full h-full text-amber-400" stroke="currentColor" strokeWidth="2">
                  <path d="M4 6h16M4 12h16M4 18h16" strokeLinecap="round" />
            ...
            <nav className="flex items-center gap-0.5 bg-slate-950 p-1 rounded-lg border border-slate-800/90 font-mono text-[11px] shrink-0">
              ...
              <button
                onClick={() => onViewChange('similar')}
                className={`px-2.5 py-1 rounded transition-all flex items-center gap-1.5 whitespace-nowrap font-bold relative ${
                  activeView === 'similar'
                    ? 'bg-amber-500 text-slate-950 shadow-sm'
                    : 'text-amber-400 hover:text-amber-300 hover:bg-slate-900 border border-amber-500/30'
                }`}
              >
                <Compass size={12} className={activeView === 'similar' ? 'text-slate-950' : 'text-amber-400'} />
