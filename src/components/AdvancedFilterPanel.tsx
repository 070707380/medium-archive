...
export const AdvancedFilterPanel: React.FC<AdvancedFilterPanelProps> = ({
  ...
  const [genreStyleSearch, setGenreStyleSearch] = useState('');

  // Extract all unique philosophical tags from dataset
  const allPhilosophicalTags = Array.from<string>(
    new Set(allItems.flatMap((item) => item.philosophicalTags || []))
  ).sort();

  // Extract all unique genre & style tags from dataset
  const allGenreStyleTags = Array.from<string>(
    new Set([
      ...allItems.flatMap((item) => item.genres || []),
      ...allItems.flatMap((item) => item.genreStyleTags || [])
    ])
  ).sort();

  // Filtered tag clouds for user search in filter panel
  const filteredPhiloTags = allPhilosophicalTags.filter((t) =>
    t.toLowerCase().includes(philoSearch.toLowerCase())
  );

  const filteredStyleTags = allGenreStyleTags.filter((t) =>
    t.toLowerCase().includes(genreStyleSearch.toLowerCase())
  );

  // Combination space mathematical estimate
  const activeFormatsCount = filters.formats.length || ALL_MEDIA_FORMATS.length;
  const activePhiloCount = filters.selectedPhilosophicalTags.length || allPhilosophicalTags.length || 1;
  const activeStyleCount = filters.selectedStyleTags.length || allGenreStyleTags.length || 1;
  const scoreCombinations = Math.max(1, filters.maxScore - filters.minScore);
  const totalCombinations = (activeFormatsCount * activePhiloCount * activeStyleCount * scoreCombinations * 2).toLocaleString();
  ...
  const allConsumedVersions = Array.from<string>(
  ...
  const allCountries = Array.from<string>(
  ...
  const allLanguages = Array.from<string>(
  ...
  return (
    <div className="bg-[#0e1117] border border-slate-800/90 rounded-xl p-3.5 sm:p-4 shadow-lg mb-6 font-mono transition-all">
      {/* Filter Header */}
      <div className="flex items-center justify-between gap-3 pb-2.5 border-b border-slate-800/80">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-xs sm:text-sm font-bold text-slate-100 flex items-center gap-2">
              ARCHIVE FILTERS
              <span className="text-[10px] px-2 py-0.2 rounded bg-amber-500/10 border border-amber-500/30 text-amber-400 font-bold">
                {matchingCount} Matches
              </span>
            </h3>
            <p className="text-[10px] text-slate-400 font-mono hidden sm:block">
              Filter by Format, Tags, Score & Decade
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {hasActiveFilters && (
            <button
              onClick={handleReset}
              className="px-2.5 py-1 rounded bg-rose-950/40 hover:bg-rose-900/60 text-rose-300 border border-rose-800/50 text-[11px] font-bold flex items-center gap-1 transition"
            >
          ...
          <button
            ...
            className="px-2 py-1 rounded bg-slate-950 hover:bg-slate-900 text-slate-300 text-[11px] border border-slate-800 transition flex items-center gap-1"
          >
            <span>{isExpanded ? 'Hide' : 'Filter Options'}</span>
        ...
        <div className="pt-3 space-y-4 animate-fade-in text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 bg-slate-950/80 p-3 rounded-lg border border-slate-800/90">
            ...
            <div>
              ...
              >
                <option value="random">Randomized Shuffle (Default)</option>
                <option value="score_desc">Hornet's Score (10 → 1)</option>
                <option value="score_asc">Hornet's Score (1 → 10)</option>
                <option value="release_desc">Release Year (Newest First)</option>
                <option value="release_asc">Release Year (Oldest First)</option>
          ...
          <div>
            <div className="flex items-center justify-between gap-2 mb-1">
              ...
              <input
                ...
                className="bg-slate-950 border border-slate-800 rounded px-2 py-0.5 text-[11px] text-slate-300 focus:outline-none focus:border-indigo-500 font-mono w-36"
              />
            </div>
            <div className="flex flex-wrap gap-1 max-h-28 overflow-y-auto p-1.5 bg-slate-950/80 rounded-lg border border-slate-800/90">
              {filteredPhiloTags.length > 0 ? (
                    ...
                    <button
                      ...
                      className={`px-2 py-0.5 rounded text-[10px] font-mono transition border ${
                        active
                          ...
                          : 'bg-indigo-950/40 hover:bg-indigo-900/60 text-indigo-300 border-indigo-900/50'
                      }`}
                    >
                      {tag}
                    </button>
                ...
                <span className="text-[10px] text-slate-500 italic p-1">No matching tags.</span>
              )}
            </div>
          </div>

          {/* Row 5: Genre & Style Tags Cloud */}
          <div>
            <div className="flex items-center justify-between gap-2 mb-1">
              <label className="text-[10px] font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-1">
                <Tag size={11} /> Style & Genre Tags ({filters.selectedStyleTags.length} active)
              </label>
              <input
                type="text"
                placeholder="Search styles..."
                value={genreStyleSearch}
                onChange={(e) => setGenreStyleSearch(e.target.value)}
                className="bg-slate-950 border border-slate-800 rounded px-2 py-0.5 text-[11px] text-slate-300 focus:outline-none focus:border-emerald-500 font-mono w-36"
              />
            </div>
            <div className="flex flex-wrap gap-1 max-h-28 overflow-y-auto p-1.5 bg-slate-950/80 rounded-lg border border-slate-800/90">
