 ...
 */
export function cleanAndCorrectTag(rawTag: string, existingTagPool: string[] = []): string {
  if (!rawTag) return '';
  ...
  const exactMatch = existingTagPool.find((t) => t.trim() === trimmed);
  ...
  if (normalizedMatch) return normalizedMatch.trim();

  // 3. Minor typo auto-correct (if input length >= 4)
  if (normInput.length >= 4) {
    ...
    for (const existing of existingTagPool) {
      ...
      if (Math.abs(normExisting.length - normInput.length) > 2) continue;

      const dist = levenshteinDistance(normInput, normExisting);
      if (dist <= 2 && dist < minDistance) {
        minDistance = dist;
        bestMatch = existing.trim();
      }
    }
...
export function processTagList(
  ...
  for (const raw of list) {
    const cleaned = cleanAndCorrectTag(raw, existingTagPool);
    if (!cleaned) continue;
